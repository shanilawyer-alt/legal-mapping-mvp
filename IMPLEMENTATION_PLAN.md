...
- **Submission lifecycle** (`domain/assessment/submission.ts`): `submitAssessmentForSession()` requires every currently-active **core** question to be answered (conditional questions stay optional even when visible — OPEN_QUESTIONS.md item 12), then flips the assessment to `SUBMITTED` and sets `submitted_at`. `assertAssessmentEditable()` (in `domain/assessment/session.ts`, shared by answer writes and document uploads) rejects any further client write with a `locked` error once an assessment leaves `DRAFT`. `reopenAssessment()` exists as an architecture-level domain function and repository port (`SUBMITTED → DRAFT`, audited) but **is not wired into any application flow** — reopen is a policy-sensitive operation whose rules (who may reopen, whether client edits should be unconditionally re-enabled, what happens to existing findings/reports) the source spec does not settle; see OPEN_QUESTIONS.md item 16, which was revised after this document's first Phase 2 draft to reflect that.
...
## 13. Phase 3 — Pilot-Readiness Flow (deterministic rules, extraction, cross-checks, findings, reports, attorney review)

Full design rationale, per-module breakdown, and the explicit out-of-scope list live in `PHASE_3_PLAN.md` (written before implementation, as instructed); this section is the completion summary. Scoped strictly to reaching `PILOT_READY` (the Phase 3 instruction's own 15-step acceptance test, now `tests/pilot-ready.test.ts`) — no post-pilot enhancement was started. Non-negotiable architecture preserved throughout: AI (synthetic-only here — no live provider, OPEN_QUESTIONS.md item 25) extracts and structures facts only; every legal finding comes from a deterministic, attorney-authored rule in `rule_catalog.csv`, never from AI judgment; no report is ever released without an explicit attorney approval + release action.

- **Canonical fact model** (`domain/facts/`): a flat `factKey → value` map, every fact carrying `sourceType`/`sourceId`/`confidence`/`createdAt` provenance. Answer-derived facts (`answer.<ID>`) are computed live from `answers` on every read, never persisted (avoids ~103 duplicate rows and staleness); document-extraction and cross-check facts are persisted `derived_facts` rows.
- **Document extraction** (`domain/extraction/`): one Zod schema per `document_analysis_matrix.csv` document type; the only implementation is `createSyntheticExtractor()` (`domain/extraction/syntheticProvider.ts`) — no live AI provider exists in this environment, so an upload with no test fixture tag always returns `status: "failed"`, never a guessed extraction (OPEN_QUESTIONS.md item 25).
- **Deterministic Rule Engine** (`domain/rules/`): all 42 `rule_catalog.csv` rules hand-translated into typed predicates (`RULE_CATALOG`), cross-validated against the live CSV at test time (Rule ID/severity/`criticalOverride`/condition text, zero drift). A rule the CSV itself marks `Manual`-tagged or whose condition has no decidable predicate is never auto-resolved (`alwaysRequiresManualReview`, OPEN_QUESTIONS.md item 21) — always routed to the attorney instead of guessed. Every evaluation stores its own `inputSnapshot` (exact facts read) for traceability.
- **Exposure/risk scoring** (`domain/rules/scoring.ts`) and **freelancer screening** (`domain/rules/freelancerScreening.ts`) implement spec §12/§13's formulas and point tables exactly, both cross-validated against `exposure_factors.csv`/`freelancer_screening_model.csv`. Freelancer screening returns a raw point total and indicator breakdown only, **no** LOW/MEDIUM/SIGNIFICANT/HIGH level — no source file defines the thresholds (OPEN_QUESTIONS.md item 27).
- **Cross-check engine** (`domain/crosscheck/`): the four spec-mandated demonstrations (employee-notice/agreement coverage, global-overtime-vs-attendance/payslip mismatch, privacy-notice completeness + monitoring-disclosure contradiction, freelancer-indicator aggregation), each producing ordinary facts (never findings directly) plus a human-readable issue for the attorney workspace.
- **Findings generation** (`domain/findings/`): one finding per matched rule (plus every `alwaysRequiresManualReview` rule, matched or not), every field copied verbatim from the catalog/scored evaluation — no generated wording. Automatic risk scoring uses only `baseSeverity`/`criticalOverride`; per-finding scope/duration/systemic/dispute inputs have no source-defined mapping from any of the 42 rules to a specific fact key, so they are never auto-computed (a documented, conservative, lower-bound design choice — OPEN_QUESTIONS.md item 28), leaving the attorney's existing severity-override action (reason mandatory) as the correction path.
- **Report generation** (`domain/report/`): structured HTML (not a binary PDF — that is master-spec Phase 6, OPEN_QUESTIONS.md item 23) rendered from persisted findings and written to the same private `DocumentStore` client documents use. The client report includes only `visibleToClient === true` findings and never Rule IDs/raw scores/legal-source links/internal notes (spec §16), matching a Hebrew band label parsed verbatim out of `report_structure.csv`'s own cell content. Three `report_structure.csv` cells call for content this system cannot honestly produce (a synthesized executive narrative, an undefined "sensitive data" classification, the already-resolved freelancer level) and are omitted rather than invented (OPEN_QUESTIONS.md item 29).
- **Pipeline orchestration** (`domain/analysis/runAnalysis.ts`): the single atomic `SUBMITTED → LAWYER_REVIEW` "Run Analysis" action (OPEN_QUESTIONS.md item 22) running the full pipeline — extraction → facts → cross-checks → rules → scored findings — single-run only (item 24).
- **Attorney review/approval/release** (`domain/review/`): the six spec §15 finding actions (confirm/modify/dismiss/override severity/add note/visible-to-client toggle) behind one `reviewFinding()`; `approveAssessment()` enforces spec §15's exact rule ("An assessment cannot be approved with unresolved CRITICAL findings in draft"); `releaseClientReport()` is the one explicit `APPROVED → CLIENT_REPORT_RELEASED` boundary, generating a final client-report snapshot at release time.
- **Attorney review workspace** (`app/(admin)/admin/assessments/[id]/page.tsx`): extended with extracted-facts/cross-check-issues/findings/reports sections and the Run Analysis/Approve/Release actions. No reopen action anywhere in Phase 3 either (OPEN_QUESTIONS.md item 16, unchanged).
- **Audit trail**: matches `MASTER_BUILD_SPEC.md`'s own `audit_events` wording exactly ("Audit login, link creation, document access, analysis, finding override, approval, report generation and deletion") — one coarse event per lifecycle action (`analysis_run`, `finding_reviewed`, `assessment_approved`, `report_generated`, `report_released`), not one row per individual fact/rule-evaluation/finding (a deliberate correction of this project's own earlier, more granular planning draft — see `domain/analysis/runAnalysis.ts`'s docstring).
- **Data-integrity finding, not fixed here** (OPEN_QUESTIONS.md item 30): `questionnaire.csv`'s `TIME-07` trigger can never be satisfied by any of `TIME-06`'s real answer options under the existing (Phase 1) exact-match branching engine, so `TIME-07`/`TIME-08` — and by extension `R-TIME-002`/`R-TIME-003` — can never fire from genuine client input today. Flagged for deliberate triage; does not block `PILOT_READY` since the rules themselves are verified correct on whatever facts they do receive.
- **Tests**: ~150 new tests across 9 new files (facts, extraction, rule catalog validation + engine, scoring, freelancer screening, cross-checks, findings, report generation, Run Analysis, review/approval/release actions) plus `tests/pilot-ready.test.ts`, the 15-step end-to-end acceptance test. Suite grew 189 → 437 across Phase 3; typecheck/lint/`next build` all clean throughout.
- **Manual verification**: same constraint as Phase 2 (item 20) — no live Supabase project is reachable here. The domain/pipeline layer (extraction → facts → cross-checks → rules → findings → scoring → reports → review/approval/release) was verified end-to-end via `tests/pilot-ready.test.ts` against the in-memory repository adapter, which is a real, complete execution of every module, not a mock of the logic. The new admin UI sections were **not** additionally driven against a real `next dev` server the way Phase 2's client flow was: admin pages sit behind `proxy.ts`'s Supabase-authenticated gate, and bypassing that (even temporarily) to smoke-test would mean editing security-critical authentication code, which was judged a disproportionate risk for the verification value versus Phase 2's client-flow swap (which only needed a DB adapter swap, not an auth bypass). Verified instead via `tsc`/`eslint`/`next build`'s own type-checked page compilation plus the fact that every Server Action the new UI calls has direct, passing test coverage of its underlying domain logic.
