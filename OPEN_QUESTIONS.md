...
## 16. Attorney reopen is scoped to `SUBMITTED → DRAFT` only

The reopen action only accepts a `SUBMITTED` assessment back to `DRAFT`. It does not attempt to handle `ANALYZED`/`LAWYER_REVIEW`/`APPROVED`/`CLIENT_REPORT_RELEASED` — those states aren't reachable by anything built through Phase 2 (they require the Rule Engine and later phases), so reopening from them is intentionally left undefined rather than guessed at now.
