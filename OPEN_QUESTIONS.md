...
## 27. Freelancer screening — no source-defined LOW/MEDIUM/SIGNIFICANT/HIGH thresholds, so no level is auto-computed; mandatory disclosure wording used verbatim in English pending an approved Hebrew translation

**Two separate gaps found while implementing spec §13** (`domain/rules/freelancerScreening.ts`):

1. **No banding thresholds anywhere in the source data.** Spec §13 says the screening "Output[s] LOW/MEDIUM/SIGNIFICANT/HIGH," and `report_structure.csv`'s own `Freelancer` row expects a client-facing "רמה בלבד" (level only) field — so a level is clearly meant to exist. But neither `freelancer_screening_model.csv` nor `MASTER_BUILD_SPEC.md` states what point total maps to which band (unlike spec §12's risk-score bands, which are given as exact numbers). Inventing a threshold split (even a "neutral" quartile of the theoretical -15..100 range) would be exactly the kind of invented legal/scoring logic your instruction forbids. **Decision**: `computeFreelancerScreening()` returns the raw total point sum and the full per-indicator breakdown (which indicators contributed and why — "Show attorney which indicators contributed," spec §13) but does **not** compute a LOW/MEDIUM/SIGNIFICANT/HIGH level at all. The attorney review workspace (task #52) shows the score and breakdown; any level assigned is the attorney's own judgment, not a system determination, until real thresholds are approved and this function is revisited.
2. **The mandatory disclosure sentence exists only in English.** Spec §13 gives it verbatim: *"The screening score reflects accumulated factual indicators and does not determine the legal status of the service provider."* No Hebrew CSV or spec section provides an official Hebrew phrasing, and every other piece of client-facing wording in this app is sourced verbatim from a CSV, never self-translated (per the "do not invent... Hebrew client wording" instruction). **Decision**: this one sentence is shown verbatim in English, exactly as the spec states it, rather than risking a self-translation that could subtly shift its legal meaning. Flag if an attorney-approved Hebrew version should replace it.
